...
Three rules follow from that, and each one produces a confusing error when you
break it.
...
`box`, `table` and `column`, print their titles regardless of that setting,
which is why the example above has headers without asking for them.
...
Getting bulk data in and out has its own commands: `.import` for CSV, `.dump`
...
lines once you set those here, and break. Recent versions print a
`-- Loading resources from ...` notice so you can see it happening. Pass
...
One caution applies to all of them. A GUI holds a connection open for as long
as the file is on screen, which leaves a `-wal` file beside the database and can
block a process that wants to write. If a write from your application fails
