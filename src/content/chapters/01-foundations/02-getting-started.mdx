...
each entry classified by a single mood and any number of tags, plus a table of
quotes and the two tables a web application needs to keep people logged in.

Twelve names appear and only seven are tables you would write queries against.
`entry_fts` is a full-text search index, and the four `entry_fts_*` names are the
shadow tables it keeps its own data in. They belong to SQLite, not to you, and

<ChapterRef id="full-text-search" /> is where they stop being clutter and start
being the point.
...
- **`mood`** and **`tag`** — small lookup tables. A mood also carries a
  ...
  into three buckets when you want the broader picture.
...
- **`quote`** — sixteen quotes, three of them anonymous. The `NULL` authors are
  deliberate, and so is the fact that some names are stored with inconsistent
  capitalisation.
- **`user`** and **`session`** — three accounts and a dozen sessions. The
  generator freezes its clock at `2026-01-15T09:00:00Z`, and exactly one session
  had expired at that instant; measured against today's real clock, all twelve
  have. Compare `expires_at` against that fixed timestamp rather than
  `datetime('now')` if you want the book's numbers. The password hashes are
  random hex and authenticate nothing.
...
The real output continues with the table's two indexes and the three triggers that
keep the search index in sync, trimmed here and covered in

<ChapterRef id="full-text-search" />.
...
on 78 entries and a tag on 14 is what makes an index on one of them worth having
...
It also means the writing is synthetic, and occasionally reads like it. Nobody's
private journal is in here, and the credentials are not credentials.

### Why it is small, and what comes after it

219 entries is a deliberate size. It is large enough that a `group by` has
something to group and a join has something to get wrong, and small enough that
you can read the whole table and check an answer by eye. For Parts II and III,
where the question is always whether a query is _correct_, that is exactly what
you want.

It is useless for performance work, and you should be suspicious of any book
that pretends otherwise. A database this size fits in memory several times over.
Every query is fast, including the badly written ones, and an index makes no
measurable difference because there is nothing to skip. Part IV therefore
switches to a second, much larger dataset, introduced when it is needed;

<ChapterRef id="choosing-indexes" /> is where the numbers start to mean
something.
...
requiring the `--experimental-sqlite` flag — a choice

<ChapterRef id="ecosystem-map" /> comes back to. `schema.sql` beside it builds
an empty copy with no rows at all, which is the version to reach for when a
...
- **`journal.db` is the book's dataset, and it is small on purpose.** 219 entries
  is enough to get a join wrong and few enough to check an answer by hand. It
  proves nothing about performance, which is why Part IV brings a bigger one.

- **The `entry_fts_*` tables are not yours.** A full-text index keeps its data in
  shadow tables that show up in `.tables`. Do not query them directly and never
  write to them.
